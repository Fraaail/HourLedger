        ...
        (function() {
            ...
            window.AndroidBridge.syncCriticalUnderHoursAlert = function(payloadJson) {
                if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.syncCriticalUnderHoursAlert) {
                    ...
                    window.webkit.messageHandlers.syncCriticalUnderHoursAlert.postMessage(normalizedPayload);
                }
            };
...
class CriticalUnderHoursAlertBridgeHandler: NSObject, WKScriptMessageHandler {
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        if let payloadJson = message.body as? String {
            CriticalUnderHoursAlertScheduler.shared.sync(fromPayloadJson: payloadJson)
            return
        }
        ...
        CriticalUnderHoursAlertScheduler.shared.cancel()
    }
}
